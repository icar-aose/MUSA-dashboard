			...
			loadGenericElement();
	 		...
	 		$.each(data.message, function(index, d){            
	 			...
	 			var idMessage=data.message[index].typeId; 
	 				...
	 				$('#messageDiv').append('<input id="typemessage'+idMessage+'" type="hidden" value="'+ data.message[index].messageType+'"/<input id="urlmessage'+idMessage+'" type="hidden" value="'+ data.message[index].url+'"/><input id="namemessage'+idMessage+'" type="hidden" value="'+ data.message[index].name+'"/><input id="exitCode'+idMessage+'" type="hidden" value="'+ data.message[index].exitCodes+'"/><table> <tr><td><img id="'+idMessage+'" src="img/message.png" width="40" height="40"/></td> <td><p>'+data.message[index].name+'</p></td></tr></table>')
	 				...
	 				$( idMessageDragg ).draggable(
	 				    		{
	 				    	       ...
	 				    	       stop:function(message,ui) {
	 				    	    	  ...
	 				    	    	  var urlMessageId="#urlmessage"+idMessage;
	 				    	    	  var urlMessage=$( urlMessageId ).val();
	 				    	    	  var typeNewMessageId="#typemessage"+idMessage;
	 				    	    	  var typeNewMessage=$( typeNewMessageId ).val();
		 				    	    	 console.log("DRAG & DROP: typeNewMessage:-->"+typeNewMessage)
	 				    	    	  addMessage(xPos,yPos,idMessage,nameMessage,urlMessage,typeNewMessage);
	 				    	       }
